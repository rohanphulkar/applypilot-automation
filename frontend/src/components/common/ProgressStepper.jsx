...
import { Check, Clock, AlertTriangle, Sparkles, FileText, Send, Save, Mail } from "lucide-react";
...
const STAGES = [
  ...
  { id: "COMPOSING_EMAIL", label: "Compose Email", icon: Mail },
    ...
    <div className="w-full space-y-5 select-none">
      ...
      <div className="flex items-center justify-between text-xs font-black uppercase tracking-wider">
        <span className="text-[#58a700] dark:text-[#a5ed6e] flex items-center gap-1.5">
          <Sparkles size={16} className="text-[#58cc02]" /> AUTOMATION LESSON PATH
        </span>
      ...
      <div className="flex overflow-x-auto no-scrollbar gap-2.5 pb-2 -mx-1 px-1 sm:grid sm:grid-cols-4 lg:grid-cols-7 sm:gap-3 sm:overflow-visible sm:pb-0 sm:mx-0 sm:px-0">
        {STAGES.map((stage, idx) => {
          ...
          let cardStyle = "bg-white dark:bg-[#1b2b32] border-2 border-[#e5e5e5] dark:border-[#2e414c] border-b-4 border-b-[#d4d4d4] dark:border-b-[#202f37] opacity-60";
          let iconBg = "bg-[#f7f9fa] dark:bg-[#233a44] text-[#afafaf]";
          let textStyle = "text-[#777777] dark:text-[#a5b6be]";

          if (isDone) {
            cardStyle = "bg-[#d7ffb8] dark:bg-[#1a3818] border-2 border-[#58cc02] border-b-4 border-b-[#46a302] opacity-100";
            iconBg = "bg-[#58cc02] text-white shadow-xs";
            textStyle = "text-[#3c3c3c] dark:text-white font-black";
          } else if (isStageFailed) {
            cardStyle = "bg-[#ffe5e5] dark:bg-[#38181a] border-2 border-[#ff4b4b] border-b-4 border-b-[#ea2b2b] opacity-100";
            iconBg = "bg-[#ff4b4b] text-white shadow-xs";
            textStyle = "text-[#ea2b2b] dark:text-[#ff7a7a] font-black";
          } else if (isCurrent) {
            cardStyle = "bg-[#ddf4ff] dark:bg-[#162a35] border-2 border-[#1cb0f6] border-b-4 border-b-[#1899d6] opacity-100 ring-2 ring-[#1cb0f6]/30";
